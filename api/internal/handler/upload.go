...
func (h *UploadHandler) InitSession(w http.ResponseWriter, r *http.Request) {
	...
	if req.FileName == "" {
		log.Warn().Msg("参数验证失败")
		response.Error(w, http.StatusBadRequest, "validation_error", "文件名为必填项")
		return
	}
	if req.FileSize <= 0 {
		log.Warn().Int64("file_size", req.FileSize).Msg("参数验证失败")
		response.Error(w, http.StatusBadRequest, "validation_error", "文件大小必须大于 0")
		return
	}
	...
	if err != nil {
		if errors.Is(err, service.ErrInvalidImageType) {
			...
			response.Error(w, http.StatusBadRequest, "invalid_type", "不支持的文件类型")
			return
		}
		if errors.Is(err, service.ErrImageTooLarge) {
			log.Warn().Err(err).Int64("file_size", req.FileSize).Msg("参数验证失败")
			response.Error(w, http.StatusBadRequest, "file_too_large", "文件大小超过限制")
			return
		}
		log.Error().Err(err).Str("operation", "InitSession").Msg("服务调用失败")
		response.InternalServerError(w, "初始化上传失败")
		return
	}
...
func (h *UploadHandler) SaveChunk(w http.ResponseWriter, r *http.Request) {
	...
	if uploadIDStr == "" {
		log.Warn().Msg("参数验证失败")
		response.Error(w, http.StatusBadRequest, "invalid_param", "缺少 uploadId")
		return
	}
	uploadID, err := uuid.Parse(uploadIDStr)
	if err != nil {
		log.Warn().Err(err).Str("upload_id", uploadIDStr).Msg("参数验证失败")
		response.Error(w, http.StatusBadRequest, "invalid_param", "无效的 uploadId")
		return
	}
	...
	if indexStr == "" {
		log.Warn().Msg("参数验证失败")
		response.Error(w, http.StatusBadRequest, "invalid_param", "缺少分片索引")
		return
	}
	index, err := strconv.Atoi(indexStr)
	if err != nil {
		log.Warn().Err(err).Str("index", indexStr).Msg("参数验证失败")
		response.Error(w, http.StatusBadRequest, "invalid_param", "无效的分片索引")
		return
	}
	...
	if err := h.uploadService.SaveChunk(r.Context(), uploadID, index, r.Body); err != nil {
		...
		if errors.Is(err, service.ErrSessionNotActive) {
			...
			response.Error(w, http.StatusBadRequest, "not_active", "上传会话不在活跃状态")
			return
		}
		...
		response.InternalServerError(w, "上传分片失败")
		return
	}
...
func (h *UploadHandler) CompleteUpload(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if errors.Is(err, service.ErrUploadIncomplete) {
			...
			response.Error(w, http.StatusBadRequest, "incomplete", "分片上传未完成")
			return
		}
		if errors.Is(err, service.ErrHashMismatch) {
			log.Error().Str("upload_id", uploadID.String()).Msg("文件哈希校验失败")
			response.Error(w, http.StatusBadRequest, "hash_mismatch", "文件哈希校验失败")
			return
		}
		if errors.Is(err, service.ErrSessionNotActive) {
			log.Warn().Str("upload_id", uploadID.String()).Msg("上传会话不在活跃状态")
		...
		response.InternalServerError(w, "合并文件失败")
		return
	}
...
func (h *UploadHandler) CancelUpload(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.uploadService.CancelUpload(r.Context(), uploadID, userID); err != nil {
		...
		response.InternalServerError(w, "取消上传失败")
		return
	}
...
func (h *UploadHandler) GetUploadStatus(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		response.InternalServerError(w, "查询上传状态失败")
		return
	}
